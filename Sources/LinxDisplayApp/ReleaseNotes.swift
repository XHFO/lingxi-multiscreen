...
enum ReleaseNotes {
    static let all: [ReleaseNote] = [
        ReleaseNote(id: "1.5.4", date: "2026-09-12", notes: [
            "新增 AI Mac 240×240 彩色小屏幕正式设备类型：最多可添加 5 台，每台独立保存连接、亮度、内容模式、推送间隔、功能卡片、画板与轮播设置。",
            "明确标注 AI Mac 小屏幕的开源硬件来源：设备本体是社区开源的 ESP8266 240×240 Wi-Fi 天气时钟（MG01 / SD2 小电视），软件内提供硬件工程与社区项目入口，多屏灵犀负责定制固件、刷写和内容推送。",
            "AI Mac 已适配灵犀68 的全部核心功能卡片：Codex、千问、番茄钟、系统监控、正在播放、自定义图片、摘录、少数派、Emoji、Home Assistant、Bambu Lab 与 Formlabs 均使用原生 240×240 高密度彩色布局。",
            "AI Mac 新增彩色多画板：从“未命名”空白画板开始，支持模块组合、自动保存、重命名、侧栏显示、拖拽排序和自动轮换；图片、专辑封面与打印机封面保留主题取色和柔和彩色光晕，不经过墨水屏灰阶、抖动或插值处理。",
            "内置 ESP8266 AI Mac 0.8.6 定制固件与 USB 刷写工具：使用 115,200 字节大端 RGB565 无损帧，并保留旧固件 24KB 内 JPEG 兼容推送。",
            "刷写前可直接填写 2.4 GHz Wi-Fi 名称与密码，固件和临时配网区一次写入；完成后自动等待设备联网、获取 IP 并添加设备，密码不保存且联网成功后由设备擦除。",
            "重做 AI Mac 热点配网页面：支持深色模式、网络扫描和选中反馈、手动填写 SSID、密码显隐、连接中与失败提示；设备热点统一为带唯一尾号的 Lingxi-Screen-XXXXXX，并明确提示不支持 5 GHz/6 GHz。",
            "AI Mac 桌面时钟改为设备端逐秒演算和绘制，软件仅负责切换与低频校时；桌面时钟同时纳入卡片管理，可控制侧栏显示、排序和自动轮播，软件预览与设备实际样式保持一致。",
            "AI Mac 正在播放新增进度条、已播放/剩余时间与设备端连续进度：封面和曲目信息低频更新，播放进度由固件本地实时推进，拖动播放位置后立即重新校准。",
            "口袋先知与 AI Mac 的正在播放页可选关联灵犀68 歌词显示；关联后目标键盘自动暂停轮播并只显示同步歌词，解除关联后键盘恢复标准正在播放画面，轮播保持用户当前设置。",
            "优化歌词时间轴：优先使用播放器提供的采样时间，按播放倍速本地推进，识别拖动跳转并及时校准；支持人工延迟微调，并根据每台设备实测上传耗时补偿换帧时刻，减少抢跑和滞后。",
            "高频歌词推送改为每台键盘仅保留最新帧；设备忙碌、休眠或重新唤醒时会丢弃已经失效的旧图像，避免恢复后按顺序补发过期歌词。",
            "AI Mac 新增亮度调节以及“始终亮屏 / 跟随 Mac 锁屏与休眠”策略；锁屏或睡眠时熄灭背光，解锁或唤醒后恢复原亮度并补推当前有效画面。",
            "设备管理新增局域网发现：可扫描 Home Assistant、口袋先知与已刷写联网的 AI Mac，按稳定身份和地址去重；灵犀68 因固件没有可靠身份接口，当前明确仅支持手动添加，避免误识别其他设备。",
            "设备管理默认进入只读保护状态，设备名称、连接字段、开关、实体映射与删除操作只有点击该设备的“编辑”后才可修改，降低误触风险。",
            "调整设备添加流程：添加灵犀68 后继续停留在设备管理页面完成配置；新键盘首次不展示全部侧栏卡片，由卡片管理引导用户按需添加。",
            "修复添加新键盘后删除它可能误删或清空旧键盘的问题：删除现在严格按稳定 UUID 执行，并恢复回退键盘自己的 IP、卡片列表、显示模式和独立设置。",
            "Home Assistant 卡片实体选择按键盘和 AI Mac 设备分别保存；切换设备不会沿用另一台设备的选择，图片与摄像头实体仍共享一次状态和静态帧刷新。",
            "Bambu Lab 自动发现改为只以完整匹配 _print_status 的打印状态实体作为每台打印机入口，再从同一设备前缀关联其余实体；修复 SD 卡状态误选、单台打印机重复添加与部分实体错绑。",
            "AI Mac 的 Bambu Lab 独立卡片为方屏重新排版，可同时展示摄像头静态帧与任务封面；X2D 等打印机的设备名、任务、状态、进度、温度和时间在有限空间内保持清晰。",
            "Emoji 壁纸按完整画布中心重新布局，整体渲染范围覆盖并适度超出屏幕边缘，放大时仅裁切画布而不会裁坏单个表情；螺旋模式不再额外补一圈外围图案。",
            "Codex 额度新增数据源切换：可使用 Codex 官方数据，或只读跟随 CC Switch 当前 Codex 供应商与用量查询规则；不会复制或保存供应商密钥。",
            "新增统一设备能力与卡片能力目录，将设备支持、数据依赖、导航、轮播和渲染配方解耦；新增显示设备或卡片时无需复制整套界面逻辑。",
            "所有屏幕继续使用最终画面指纹去重：内容没有变化时跳过重复推送；切换设备、唤醒、首次连接或修改关键显示设置时仍保证有效首帧及时送达。",
        ]),
